...
export default function Widget() {
    ...
    if (error) return (
        <div class="w-full max-w-md p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-8 dark:bg-gray-800 dark:border-gray-700">
            ...
            <br></br>
            <ErrorMessageContainer error={error} message="We weren't able to get the latest Recent Repayments."/>
        </div>
    ...
    return (
        <div class="w-full max-w-md p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-8 dark:bg-gray-800 dark:border-gray-700">
            ...
            { Array.isArray(data) && data.length > 1? data.map((info) => {
                let { data } = info;
                ...
                <li class="py-3 sm:py-4">
                    <div class="inline-flex items-center text-base font-semibold text-gray-900 dark:text-white">
                        <MessageContainer message={`No recent payment updates for ${username}`}/>
                    </div>
                </li>
            </ul>
        </div>}
